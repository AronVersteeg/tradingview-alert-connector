...
Every active dashboard pair can arm independent long and short manual entry plans with separate optional locked TP ladders:

```text
MANUAL_ENTRY_TP_OVERRIDE_ENABLED=true
# Optional when /app/data is not already persistent. Other pair files use the same directory:
BTC_MANUAL_TRADE_OVERRIDE_FILE=/app/data/btc-manual-trade-override.json
```

The plans use each pair's Binance Futures 1H candles and wait for the first fresh close above a long trigger or below a short trigger. Multiple plans in the same direction and an opposite-direction plan can remain armed together; triggering one plan does not cancel the others. They never act retroactively: only candles closed after arming qualify, and execution must begin within 15 minutes of the latest close. While a pair has an armed or executing plan it has priority over automatic Delay and Shadow entries for that pair. A supplied TP ladder remains fixed for that position; without manual TPs the existing dynamic map TP logic applies. Position sizing, dynamic risk downsizing, the initial dYdX 1H Williams-fractal stop and trailing-stop automation are the same as for the pair's normal managed entries. A pair-specific `MANUAL LONG TRIGGERED` or `MANUAL SHORT TRIGGERED` email records the candle, execution result, SL and TPs. Arming, editing and cancelling use `DECENTRADER_API_TOKEN` (or the existing `TRADINGVIEW_PASSPHRASE` fallback), and an executing plan cannot be changed.
