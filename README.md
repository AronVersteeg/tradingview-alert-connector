...
Set `DECENTRADER_INTRUSION_CANDLE_FILTER_ENABLED=true` to gate Decentrader auto-trades behind an all-closed-candles confirmation using dYdX 1H BTC-USD candles. Raw gap-intrusion emails keep their normal subject and are always sent for audit/replay, while confirmed entries also send a second `FILTERED ...` email before the entry flow. The normal email's SMTP acceptance time fixes the end of The Delay window. A right-edge intrusion requires the intrusion candle and every following fully closed candle before that cutoff to be red; a left-edge intrusion requires all of them to be green. At least two closed candles are required, an open candle at the cutoff is excluded, and one mismatching candle fails the filter. The dYdX candles are authoritative and do not require Decentrader to publish each following timelapse frame. Missing dYdX candle data keeps the alert in a pending review queue without resending a successfully accepted raw mail. When a delayed review passes, the historical frame proves the signal only: entry, gap, fractal SL, TP ladder and sizing are rebuilt from the current map and dYdX state. `DECENTRADER_INTRUSION_MAX_EXECUTION_DELAY_HOURS` and `DECENTRADER_INTRUSION_MAX_PRICE_DRIFT_PCT` bound The Delay before a live order can be placed. Leave the filter `false` to keep the original immediate intrusion-alert setup. The map also has a local `Candle on/off` toggle for replay comparison.
...
The Public Perp V2 panel is a separate observe-only replacement track for the original synthetic public study. It causally reconstructs position cohorts from public Binance, Bybit and OKX hourly open-interest changes, using Binance taker flow plus venue candle direction to infer which side opened. Later OI reductions shrink only existing venue cohorts and later candle sweeps remove crossed liquidation levels, so historical frames never use future prices. Hyperliquid and dYdX provide independent price/volume context. GMX public positions are monitored, but stay outside the primary histogram while their liquidation prices remain model estimates. A gap is derived only from the combined histogram density: the nearest material edges must leave at least 72% clean interior space, otherwise no green gap is rendered. V2 persists hourly observations and never imports or calls the Decentrader execution path.
...
OPEN_LIQUIDITY_V2_MIN_CLUSTER_USD=1000000
OPEN_LIQUIDITY_V2_MIN_GAP_CLEANLINESS=0.72
```
